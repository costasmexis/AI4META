...
class MLPipelines(MachineLearningEstimator):
    ...
    def _one_sem_model(self, trials, model_name):
        ...
        simplest_model = min(filtered_trials, key=lambda x: __model_complexity(x['params']))
    ...
    def _filter_features(self, train_index, test_index, X, y, num_feature2_use):
        ...
        X_train, X_test = self.normalize(X=X_tr,train_test_set=True,X_test=X_te, method=self.config_rncv['normalization'])
        
        # Manipulate the missing values for both train and test sets
        X_train = self.missing_values(data=X_train, method=self.config_rncv['missing_values_method'])
        X_test = self.missing_values(data=X_test, method=self.config_rncv['missing_values_method'])
        ...
        if self.config_rncv['feature_selection_type'] != 'percentile':
            if type(num_feature2_use) is int:
                if num_feature2_use < X_train.shape[1]:
                    self.selected_features=self.feature_selection(X=X_train, y=y_train,method=self.config_rncv['feature_selection_type'], num_features=num_feature2_use, inner_method=self.config_rncv['feature_selection_method'])                            
            ...
            if type(num_feature2_use) is int:
                if num_feature2_use == 100 or num_feature2_use == self.X.shape[1]: #TODO: check how to write it better
                    ...
                    self.selected_features=self.feature_selection(X=X_train, y=y_train,method=self.config_rncv['feature_selection_type'], inner_method=self.config_rncv['feature_selection_method'], num_features=num_feature2_use)
                    X_train_selected = X_train[self.selected_features]
                    X_test_selected = X_test[self.selected_features]
                    num_feature=num_feature2_use
                else: 
    ...
    def inner_loop(self, train_index, test_index, X, y, avail_thr):
        ...
        for num_feature2_use in feature_loop:
            X_train_selected, X_test_selected, num_feature = self._filter_features(train_index, test_index, X, y, num_feature2_use) 
            ...
            if self.config_rncv['clfs'] is None :
                ...
                for estimator in self.config_rncv['clfs']:
                    ...
                    clf = optuna.integration.OptunaSearchCV(estimator=self.estimator, scoring=self.config_rncv['inner_scoring'],
                                                            param_distributions=optuna_grid[opt_grid][self.name],
                                                            cv=self.config_rncv['inner_cv'], n_jobs=n_jobs, verbose=0, n_trials=self.config_rncv['n_trials_ncv'])
                    ...
                    if self.config_rncv['inner_selection'] == 'validation_score':
                        results['Scores'].append(get_scorer(self.config_rncv['outer_scoring'])(clf, X_test_selected, y_test))
                        ...
                        # Fit the new model
                        new_params_clf = self._create_model_instance(self.name, simple_model_params)
                    ...
                    if num_feature == 'full' or num_feature is None:
                        ...
                        results['Classifiers'].append(f"{self.name}_{self.config_rncv['feature_selection_type']}_{num_feature}")
                        results['Selected_Features'].append(X_train_selected.columns.tolist())  
                        results['Number_of_Features'].append(num_feature)
                        results['Way_of_Selection'].append(self.config_rncv['feature_selection_type'])
    ...
    def outer_loop(self,i,avail_thr):
        ...
        self.config_rncv['inner_cv'] = StratifiedKFold(n_splits=self.config_rncv['inner_splits'], shuffle=True, random_state=i)
        self.config_rncv['outer_cv'] = StratifiedKFold(n_splits=self.config_rncv['outer_splits'], shuffle=True, random_state=i) 
        ...
        if self.config_rncv['parallel'] == 'freely_parallel':
            temp_list = []
            with progressbar.ProgressBar(prefix = f'Outer fold of {i+1} round:',max_value=self.config_rncv['outer_splits'], widgets=widgets) as bar:
                split_index = 0
    ...
    def nested_cv(self,n_trials_ncv=25,rounds=10, exclude=None, freq_feat=None, search_on=None,
        ...
        if parallel == 'thread_per_round':
            ...
            with threadpool_limits(limits=avail_thr):
                list_dfs = Parallel(n_jobs=use_cores,verbose=0)(delayed(self.outer_loop)(i,avail_thr) for i in trial_indices)
