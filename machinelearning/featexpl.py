...
class FeaturesExplanation(MachineLearningEstimator):
    ...
    def calculate_shap_values(self,explainer_type='general'):
        ...
        if explainer_type == 'general':
            self.explainer = shap.Explainer(self.best_estimator.predict, self.X)
            ...
            if self.name not in ['DecisionTreeClassifier', 'RandomForestClassifier', 'XGBClassifier']:
                ...
                self.explainer = shap.TreeExplainer(self.best_estimator.predict, self.X)
            ...
            if self.name not in ['LogisticRegression', 'LinearDiscriminantAnalysis']:
                ...
                self.explainer = shap.LinearExplainer(self.best_estimator.predict, self.X)
        else:
    ...
    def plot_shap_values(self,max_display=10,plot_type='summary'):
