# In src/utils/validation/validation.py
...
# class ConfigMS:
#     """Configuration manager for model selection parameters"""
    
#     def __init__(self):
#         """Initialize configuration manager"""
#         # Validation settings
#         self.valid_scorers = list(get_scorer_names()) + ["specificity"]
#         self.valid_model_types = ["rcv_accel", "rnested_cv", "both"]
        
#         # Initialize logging
#         self.logger = logging.getLogger(__name__)
#         self.logged_messages = set()
    
#     def validate(self, config: ModelSelectionConfig, X: pd.DataFrame, csv_dir: str) -> ModelSelectionConfig:
#         """
#         Validate configuration and return corrected version
...
#         X : DataFrame
#             Input features to validate against
#         csv_dir : str
#             Path to CSV file
...
#         # Validate components
#         self._validate_model_selection_type(validated_config)
#         self._validate_num_features(validated_config, X)
#         self._validate_scoring_metrics(validated_config)
#         self._configure_classifiers(validated_config)
#         self._extract_dataset_info(validated_config, X, csv_dir)
        
#         return validated_config
    
#     def _validate_model_selection_type(self, config: ModelSelectionConfig) -> None:
#         """Validate model selection type"""
#         if config.model_selection_type not in self.valid_model_types:
#             self._log_once(f"Invalid model selection type: {config.model_selection_type}. Using default: both")
#             config.model_selection_type = 'both'
    
#     def _validate_num_features(self, config: ModelSelectionConfig, X: pd.DataFrame) -> None:
...
#         elif not isinstance(config.num_features, list):
...
#         #         raise ValueError("For rcv, num_features must be an integer or None")
    
#     def _validate_scoring_metrics(self):
#         """Validate scoring metrics"""
#         if self.scoring not in self.valid_scorers:
#             raise ValueError(f"Invalid scoring metric: {self.scoring}. "
#                            f"Valid options: {self.valid_scorers}")
        
#         # if self.model_selection_type in ['rncv', 'both']:
#         if self.inner_scoring not in self.valid_scorers:
...
#         csv_dir = self.ml_selector.csv_dir
#         X = self.ml_selector.X
        
#         # Extract dataset name from path
#         pattern = r"[^/]+(?=\.csv)"
#         match = re.search(pattern, csv_dir)
#         self.dataset_plot_name = match.group() if match else ''
...
#         self.all_features = X.shape[1]
    ...
    #     This provides backward compatibility with code expecting a config dictionary.
    #     """
    #     return {k: v for k, v in self.__dict__.items() 
    #             if not k.startswith('_') and k != 'ml_selector' 
    #             and k != 'logger' and k != 'logged_messages'}

# class ConfigValidator(MLSelector):
#     """
