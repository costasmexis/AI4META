...
datasets = ['gastric_cancer', 'chronic_fatigue','lung_cancer','periodontal_inflammation','epic_lc_ms_neg','epic_lc_ms_pos','epic_ce_ms','epic_gc_ms','epic_composite']
inner_selection = ['one_sem','validation_score','gso_1','gso_2','one_sem_grd']

for dataset in datasets:
    csv_dir = 'data/' + dataset + '.csv'
    if (dataset == 'epic_ce_ms') or (dataset == 'epic_lc_ms_neg') or (dataset == 'epic_lc_ms_pos') or (dataset == 'epic_gc_ms') or (dataset == 'epic_composite'):
        label = 'group'
    else:
        label= 'Class' 
